...
def plot_bar_chart(labels, values):  # Gets two lists as input
    ...
    fig = go.Figure(
        go.Bar(x=labels, y=values, marker=dict(color=values, colorscale="Viridis"))
    )
...
def plt_hist(data, label, title, xaxis_range=[0, 200]):
    ...
    xticks = list(
        range(xaxis_range[0], xaxis_range[1] + 1, 10)
    )  # 10 minutes ticks with 200 of range gives nice results
    ...
    fig.update_layout(
        xaxis=dict(
            ...
            ],  # Just to get the minutes values in text
        )
    )
