...
with open('/home/onyxia/work/projet_python_ds2024/app/data/movies_plots.json', "r") as file:
...
with st.sidebar: # Button on sidebar looks nicer I think 
    if st.button("Recommend me movies"):
        ...
        if valid_input:
            liked_movies_indexes = [movie_list.index(movie) for movie in final_input if movie in movie_list]
            st.session_state["recommendations"] = recommend_movies(pop_films, liked_movies_indexes)
        else:
