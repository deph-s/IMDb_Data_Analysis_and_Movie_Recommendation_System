...
st.write('''We focus here on the dataset of the popular entries, this means that, here, we also include TV Shows, 
hence 'entry' length''')
...
st.write('''We're back to our original dataset of popular movies, does including TV Shows which typically have shorter episodes
change the average durations of some genres that much ?''')

average_runtime_per_genre_f = avg_runtime_genre(pop_films)

labels_f = average_runtime_per_genre_f['Genres'].tolist() # The labels are simply the genres here
values_f = average_runtime_per_genre_f['Runtime'].tolist()

fig_f = plot_radial_chart(labels_f, values_f)
st.plotly_chart(fig_f)
st.write('''Not that much for many genres, however we can notice those facts : 
...
pop_films = pop_films.dropna(subset=['Released', 'Rating'])  # Drop rows with no rating or no release date
...
fig, pval, rsquared, beta = plot_line_chart(average_ratings_by_year, 'Year', 'Average Rating', 'Average rating per year of popular films')
st.plotly_chart(fig)
...
st.plotly_chart(fig)
st.markdown("""<p style='text-align: center;'>⤷ There is a lot more content that is being released today than in the 
                past so probably more content of lower quality. </p>""", unsafe_allow_html=True)
st.markdown("""<p style='text-align: center;'>⤷ People that watch older movie probably watch movies that are known 
                to be good thus making them last in time.</p>""", unsafe_allow_html=True)
...
imdb_data = imdb_data.dropna(subset=['Released', 'Rating'])  # Drop rows with no rating or no release date
average_ratings_by_year = imdb_data.groupby('Released')['Rating'].mean().reset_index()
average_ratings_by_year.columns = ['Year', 'Average Rating']
average_ratings_by_year = average_ratings_by_year.sort_values(by='Year')

fig, pval, rsquared, beta = plot_line_chart(average_ratings_by_year, 'Year', 'Average Rating', 'Average rating per year of all the entries')
st.plotly_chart(fig)
