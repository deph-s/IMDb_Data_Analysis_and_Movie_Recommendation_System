...
def get_raw_data(): # Adapt this function with the new data links 
    ...
    fs = s3fs.S3FileSystem(
        client_kwargs={"endpoint_url": "https://" + "minio.lab.sspcloud.fr"}, # If ran on SSP Cloud, keys are already filled in
    )

    MY_BUCKET = "keithmoon" # My ID on SSP Cloud (Antoine)
    fs.get(f"{MY_BUCKET}/diffusion/title.basics.tsv.gz", "title.basics.tsv.gz") # Get the compressed files from S3 
    ...
    titles_df = pd.read_csv('title.basics.tsv.gz', sep='\t', compression='gzip') # Basic infos, title, release, runtime, genre
    crew_df = pd.read_csv('title.crew.tsv.gz', sep='\t', compression='gzip') # Adds infos on director, writer etc... 
    ratings_df = pd.read_csv('title_ratings.tsv.gz', sep='\t', compression='gzip') # Gives us critics scores
    episodes_df = pd.read_csv('title.episode.tsv.gz', sep='\t', compression='gzip') # Infos on tv shows episodes

    films_ = pd.merge(titles_df, crew_df, on ='tconst') # Merge on unique film identifier
    films__ = pd.merge(films_, ratings_df, on='tconst')
    films = pd.merge(films__, episodes_df, on='tconst', how='left') # Left join because I want to add the episode id in case it exists, if it doesn't I just don't touch the row, this will allow me to remove from the dataframe any row that has a episode id filled in so that I can specifically exclude episodes of tv shows from the dataframe

    films = films.drop(columns=['titleType', 'originalTitle', 'isAdult','endYear','writers']) # Don't care

    films.rename(columns={'tconst':'film_id', # Giving better names to columns
                      'primaryTitle':'Title', 
                      'averageRating':'Rating', 
                      'startYear':'Released',
                      'runtimeMinutes':'Runtime', 
                      'genres':'Genres', 
                      'directors':'Directors'}, inplace=True)
    
    films['Released'] = pd.to_numeric(films['Released'], errors='coerce') # For some reason unknown to man released and runtime weren't int
...
def remove_tv_episodes(data):
    data = data[data['parentTconst'].isna()] # Remove row if a TV Show name is given as it is an episode
    return data
...
def keepfilmsonly(data, start_from=0):
    ...
    for index, row in to_process.iterrows():
        imdb_id = row['film_id']
        movie = ia.get_movie(imdb_id[2:])  # imdb_id starts with 'tt' and cinemagoer only uses the digits that come after
        with open('output.txt', 'a') as file: # Initially I just dumped all of that in a simple txt file and filtered with the ids after
    ...
    with open('output.txt', 'r') as file:
        for line in file:
            id = line.strip().split(":")[-1] # Because the txt is composed of lines like : "The Godfather : tt62658513"
            id_list.append(id.strip())

    data = data[data['film_id'].isin(id_list)] # Only keep the rows that have their id present in the id list of films
    return data

def get_films_plots(data, omdb_api_key, start_from=0): # Used a ton of API keys because of daily request requirements, wanted to speed up the process
    request = f'http://www.omdbapi.com/?i=tt3896198&apikey={omdb_api_key}' # To adapt
    to_process = data[data.index >= start_from]
    ids = to_process['film_id'].tolist() # Simply convert the film_ids to a list to loop over it
    ...
    with open("movies_plots.json", "r+") as json_file: # r+ to append to the file and to not overwrite all the work so far
        current_data = json.load(json_file)

        for id in ids:
            response = requests.get(f'http://www.omdbapi.com/?i={id}&apikey={omdb_api_key}')
            
            if response.status_code == 200: # 200 is a positive answer for the server
                data = response.json()

                if data.get("Response") == "True": # We get a dictionnary where Response is a key 
                    plot = data.get("Plot", "No plot available") # If there's no plot we default to writing "No plot available"
                    
                    current_data.append({ # Json is not great for really large file as we are reading the entire file every time 
                    ...
                    json.dump(current_data, json_file, indent=4) # We overwrite the previous file with the new data
...
def scrape_film_reviews(imdb_id): # It scrapes the 10 first reviews for the specific film whose id is inputted 
    ...
    if response.status_code != 200: # 200 is that everything went well, but if it didn't went well we signal it
        print(f"Failed to fetch data: {response.status_code}")
        return []
    
    soup = BeautifulSoup(response.text, 'html.parser') # We use the native html parser and use bs to make the html easier to process
    reviews = []
    
    review_containers = soup.find_all('div', class_='ipc-html-content-inner-div') # If you inspect the HTML code of the review page on IMDb, you see that the reviews are stored in div with the parameter that I entered
    ...
    for i, review in enumerate(review_containers):
        ...
        time.sleep(0.5)  # Avoid getting ip banned because of too many requests 0.5 seconds seems to be the shortest safe timeout
...
def get_json_reviews(data, starting_index=0): # Same as always, it takes a while so we can resume the process at a specific index
    to_process = data[data.index >= starting_index]
    ids = to_process['film_id'].tolist()

    json_file_path = "movies_reviews.json" # Convenient to create the file if it doesn't exist yet
    if not os.path.exists(json_file_path):
        with open(json_file_path, "w") as json_file:
    ...
    with open("movies_reviews.json", "r+") as json_file: # r+ to append to the file and to not overwrite all the work so far
        current_data = json.load(json_file)
        for id in ids:
            reviews = scrape_film_reviews(id) # Takes a while because of the hidden time.sleep in this function
            current_data.append({ # Json is not great for really large file as we are reading the entire file every time 
...
def preprocess_text(text): # Using regular expression, remove additional spaces, punctuation, lowercase the text and remove all special characters to get raw strings on which we can do text embeddings 
    
    try:
        text = json.loads('"' + text + '"') # Sometimes there are special characters that are escaped, apparently this is the way to handle them properly
...
def preprocess_plots(): # Obviously to run this function there should be a json file of plots to process
    ...
    processed_data = []
    for entry in data:
        plot = entry.get("Plot", '') # There is always a plot in our database anyways but I added a default behaviour
    ...
    with open('processed_plots.json', 'w') as outfile: # We just dump all that we processed in the new file
...
def preprocess_reviews():  # Obviously to run this function there should be a json file of reviews to process
    ...
    processed_data = []
    for entry in data:
        reviews = entry.get("Reviews", '')  # There is always a plot in our database anyways but I added a default behaviour
        if isinstance(reviews, list):  # Handle the case where reviews might unexpectedly be a list
    ...
    with open('processed_reviews.json', 'w') as outfile:  # We just dump all that we processed in the new file
        json.dump(processed_data, outfile, indent=4)

    print("Processed JSON saved to 'processed_reviews.json'")

def attach_text(data): # Quick function to add a new column to the dataframe with the text of plots and reviews concatenated
    ...
    for plot, review in zip(processed_plots, processed_reviews):
        ...
        reviews_text = str(review.get('Reviews', '')) # Don't know why reviews was a list of characters, changed it to a string
...
data = data[:50] # To process the scraping faster, 50 because it happens that a vast majority of the top rated entries are tv shows
...
data = attach_text(data) # After processing all of the text we can finally attach it, then after that as seen above, embeddings are computed on this column.tolist()
print("Done !")
print("Not displaying the last step as the reviews and plots are too long to be properly displayed in the command line")
